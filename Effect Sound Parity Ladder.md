...
- **As of iter_0452 all 7 effect-sound parity ladder positions PASS: each position adds one SMD opcode to the minimal Cure SMD — `0xBA` ReverbOn, `0xDB`, `0xB0` SlurOn, `0xC4` ADSR_SustainRate, `0xD1` AddPitchBend, `0xD4` (2-param) — and the Godot synth renders every position with all manifest probes paired on the same session and audio cos_dist below the 0.09 floor.** — `[S·D·R] 3/3`
  ...
  - S: `smd_release` @ `0x800161E0` (0xC4) — ADSR_SustainRate identity re-confirmed at ladder pos 5; the opcode was previously misdocumented as "Release/decay" — `research/effect_sound/working_documents/SPU_VOICE_LIFECYCLE.md` (2026-06-11)
  ...
  - D: ladder snapshot in PROBE_TO_DISASM.md (2026-05-03): positions 2–7 add 0xBA (iter_0435), 0xDB (iter_0448), 0xB0 SlurOn (iter_0449), 0xC4 ADSR_SustainRate first parameterized (iter_0450), 0xD1 AddPitchBend 1-param tier complete (iter_0451), 0xD4 first 2-param (iter_0452) — each addition keeps all 8 GOLD probes PAIRED + audio cos_dist below the 0.09 floor
