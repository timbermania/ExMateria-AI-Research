...
- **Particle physics parameters: inertia 0x08 with formula `velocity = ((inertia − 560) × velocity + accel) / inertia` (higher inertia resists both slowing and speeding up), weight 0x0A with `velocity_y += weight` (0 = floats, 10 = falls 10 units/frame), drag coefficients 0x30–0x38, and homing target_x/y/z 0x3C–0x40 (int16, computed at spawn from anchor + offset).** — `[S] 1/3`
  ...
  - src: `research/key_documents/STRUCTURE_DEFINITIONS.md`
- **Particle lifetime and behaviour: lifetime_counter 0x42 (decremented each frame; 0xFFFF = die by animation duration instead), homing_curve_index 0x45, color curve indices 0x46–0x48 (lookup 0x80 = normal, <0x80 = darker, >0x80 = brighter), homing_strength 0x4A (0 = simple physics, !=0 = enable homing — NOT a countdown), motion_flags 0x4C (copied from emitter bytes 0x02–0x03; bit 1 align_sprite_to_screen_velocity, bits 12–15 handler_index), behavior_flags 0x4E (copied from emitter bytes 0x06–0x07), anim_frame_counter 0x50 (wraps at 160), child_emitter_on_death/mid_life 0x52/0x53, and anim_state 0x54.** — `[S·R] 2/3`
  - S: Particle field offsets 0x42–0x54, per `research/key_documents/STRUCTURE_DEFINITIONS.md`
  - S: `emitter_control_routine` decompilation — emitter offsets 0x94–0x9A (hidden in `reserved_64_AF`) lerp into particle+0x42, which the function at 0x801a3064 decrements each frame; at 0 the particle dies and its on-death child emitter spawns — per `research/working_documents/LIFETIME_CORRECTION.md`
  - S: `integrate_particle_motion` decompilation — particle+0x4A comes from emitter 0xB8–0xBE (homing_param_min/max; the Ghidra "lifetime" names are misleading): 0 = simple accel+drag physics, !=0 = target-seeking toward particle target 0x3C–0x40 with the value scaling homing force; never decremented — per `research/working_documents/LIFETIME_CORRECTION.md`
  - S: BATTLE.BIN 0x801a3048 — the 0xFFFF branch skips the countdown: loads the anim_state pointer from particle+0x54, reads the int16 duration at anim_state+0x16 and uses it for the death check, so the lifetime field is never decremented in animation-driven mode (0x801a3064 is the countdown branch that decrements particle+0x42), per `research/working_documents/WIKI_PARTICLE_LIFETIME.md`
  - R: `godot-learning/src/effects/Particle.gd:87-92` (particle dies at age >= lifetime; -1 = animation-driven) + `godot-learning/src/effects/ActiveEmitter.gd:149-154` (lifetime lerped from emitter min/max start/end fields) and `:194-197` (homing target set only when homing_strength > 0); validated by `godot-learning/tests/EffectSoundCaptureTest.gd`
  - src: `research/key_documents/STRUCTURE_DEFINITIONS.md`
  - src: `research/working_documents/LIFETIME_CORRECTION.md`
  - src: `research/working_documents/WIKI_PARTICLE_LIFETIME.md`
