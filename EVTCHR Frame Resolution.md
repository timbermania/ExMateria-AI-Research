...
- **Each EVTCHR.BIN file block (0x7800; 137 blocks × 0x7800 = the whole file) holds: cinematic SEQ table @ `0x0000` (`0x500`), a 40-entry frame-pointer table (LE32) @ `0x0500`, SHP-TYPE1 4-byte tile descriptors @ `0x05A0`, 16 palettes × 16 colors (BGR555) @ `0x0780`, and a 256×200 4bpp pixel page (`0x200`) with `0xFF` padding @ `0x7000`; a "frame" is one frame-pointer entry → a run of SHP-TYPE1 tile descriptors → tile UVs into that page, so "row/column" is the tile-UV grid.** — `[S·R] 2/3`
  ...
  - R: `godot-learning/tools/parse_evtchr.py` `decode_pixels` — 4bpp byte holds 2 pixels, low nibble = left pixel (PSX convention), pinned by `godot-learning/tools/test_parse_evtchr.py` (`test_segment_0_pixel_indices_are_4bpp`)
  - src: `research/working_documents/EVTCHR_FRAME_RESOLUTION.md`
...
- **The EVTCHR block-pointer table's runtime frame index is `disc_byte + 7`: the disc table is authored for frame bytes `0xCB..0xF2` but the runtime rewrites the data to `0xD2..0xF9` because the dispatcher `FUN_80083F18` uses `frame_byte >= 0xD2` (`sltiu v0,s2,0xd2`) as the EVTCHR-vs-TYPE1.SHP threshold, so `block_frame_idx = frame_id − 0xD2`; it is a frame-INDEX offset, not an X shift, and never touches `tile_x`.** — `[S·D·R] 3/3`
  ...
  - D: live pointer-table dump, `orbonne_prayer_mid_dialog` after one Cross press (2026-06-25): runtime `0xD2`/`0xD9`/`0xE7`/`0xF3`/`0xF4`/`0xF9` → disc `0xCB`/`0xD2`/`0xE0`/`0xEC`/`0xED`/`0xF2` (src `(0,0)`/`(0,40)`/`(0,120)`/`(160,160)`/`(192,160)`/`(128,200)`) — consistent +7 across all six samples
  - src: `research/working_documents/EVTCHR_FRAME_RESOLUTION.md`
...
- **The EVTCHR branch of `FUN_80083F18` (`LAB_80084194`) resolves the frame descriptor from runtime pointer tables: block 2 `0x800AF244` is the default, with `sltiu v0,anim_id,0x258` @ `0x800841a0` (`addiu s1,s1,-0x7564`) falling back to block 1 `0x800A7CE0` for `anim_id < 0x258`; the pointer is `*(base + frame_byte * 4)` — the raw runtime frame byte, no subtraction — and is stashed in `*s6` for `FUN_80084214`'s tile assembly; in the chapel block 2 holds 40 valid ptrs across `0xD2..0xF9` and block 1 is all zeros.** — `[S·D·R] 3/3`
  - S: `LAB_80084194` (`0x80084194..0x800841e0`: block select, `sltiu` @ `0x800841a0`, `v0 = *(s1 + frame_byte*4)` @ `0x800841c4..c8`, stash `*s6 = v0` @ `0x800841e0`), tables `0x800AF244`/`0x800A7CE0` (`battle_disassembly.txt`)
  - D: live pointer-table dump, `orbonne_prayer_mid_dialog` after one Cross press (2026-06-25): `0xD2`→`0x800AFAAC`, `0xD9`→`0x800AFAD6`, `0xE7`→`0x800AFB2A`, `0xF3`→`0x800AFB72`, `0xF4`→`0x800AFB78`, `0xF9`→`0x800AFB96`, `0xFA`→`0` (table end)
  - R: `godot-learning/tools/parse_evtchr_frames.py` (`FRAME_ID_BASE = 0xD2`, `block_frame_idx = frame_id − 0xD2`; docstring cites `FUN_80083f18` @ `0x80083FC8`) + `godot-learning/tools/test_parse_evtchr_frames.py` (`DecodeBlockUnit`)
  - src: `research/working_documents/scenario_1_captures/cinematic_frame_offset_decode.md`
- **The chapel mid-dialog visible pose is Ovelia's anim `0x025C` (RAM bytecode `E7 08 08 02 FF FF`): first dispatch `0xE7` (≥ 0xD2) resolves through the block-2 EVTCHR table to disc frame `0xE0` (src `(0,120)`), second `0x08` (< 0xD2) falls to the unit's own TYPE1.SHP frame 8, then PauseAnimation — so the user-visible "kneel" is the unit's TYPE1 frame 8, NOT segment-0's `0xCE`/`0xCF` (her `0x263` nod anim `F3 06 F4 02` never fires in the captured run); the narrator's standing pose (Gafgarion per the capture doc, slot 12, sprite-set `0x000C`) is anim `0x0258` → frame `0xD9` (disc `0xD2`, src `(0,40)`), offset 0.** — `[S·D·R] 3/3`
  - S: band-split `sltiu` @ `0x80083FC8`, block-2 pointer table `0x800AF244` (`battle_disassembly.txt`)
  - D: capture `orbonne_prayer_mid_dialog` → one Cross press (2026-06-25): PCSX ground-truth `/tmp/pcsx-agent-shots/pcsx-2504155/143611_824_pause.png` (visible bowing), live anim_state RAM (slot 1 `01 00 00 00 5c 02 02 00 e7 00 08 00` — anim `0x025C`, +0x08 `0xE7`; slot 12 — anim `0x0259`, +0x08 `0xD5`), `tools/parse_evtchr.py` disc decode
  - R: `godot-learning/src/scenarios/ScenarioVM.gd` (cinematic walker `_render` band split: `fb >= 0xD2` → `enter_cinematic_mode` + `load_cinematic_frame(seg_id, fb, atlas_y_offset)`, else exit cinematic mode + `load_frame_by_id(TYPE1)`; the `enter_cinematic_mode` snapshot exists for exactly this `0x25C` TYPE1 flip) + `godot-learning/tests/ScenarioCinematicWalkerTest.gd` (23/0, incl. `_test_walker_routes_low_byte_to_type1`)
  - src: `research/working_documents/scenario_1_captures/cinematic_frame_offset_decode.md`
