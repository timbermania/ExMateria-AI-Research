...
- **The 4-bit curve-index nibbles are 1-based: stored value 0 means no curve and values 1–15 select curves 0–14 (curve_index = nibble − 1) — E001 emitter 2's particle-count nibble (byte 0x0E high nibble) read 0x3 → curve 2, and the written 0x1 → curve 0.** — `[D·R] 2/3`
  ...
  - D: E001 emitter 2 spawn-position nibble (byte 0x08 low) runtime write test (2025-11-20): original `0x2` at 0x801C298C selected curve 1, written `0x1` selected curve 0 (smooth ramp) and the spawn sweep followed the ramp
...
- **E001's animation-curve table at 0x801C2D58 (pointer global 0x801BBF7C holds `58 2D 1C 80`) holds 15 curves of 160 bytes each, curve N at base + N×0xA0: curve 0 is a smooth ramp 0x00→0xFA over its 160 frames (0%→98%) and curve 2 is a step function (0x00 at frames 4–66, 0xFF at frames 67–129, 3 transition bytes at each end) — burst vs gradual spawning for the particle-count curve.** — `[S·D] 2/3`
  ...
  - src: `research/working_documents/VERIFIED_particle_count_curve.md`
- **The spawn-position curve (low nibble of emitter byte 0x08) is resolved by FUN_801a8c14(emitter, factor, &out), which writes the interpolated spawn-center position into the particle's 12.12 position (+0x0C/+0x10/+0x14).** — `[S·D·R] 3/3`
  - S: FUN_801a8c14 call and particle +0x0C/+0x10/+0x14 writes in FUN_801a60ac, per `research/working_documents/CURVE_ANALYSIS.md`
  - S: FUN_801a8c14 lerp formula `spawn[axis] = start[axis] + (end[axis] − start[axis]) × curve[anim_frame] / 255`, computed once per axis (X, Y, Z) with the same interpolation factor; start int16s at emitter +0x14/+0x16/+0x18, end int16s at +0x1A/+0x1C/+0x1E, per `research/working_documents/VERIFIED_spawn_position_curve.md`
  - D: E001 emitter 2 runtime write tests (2025-11-20): start/end X (−100/+100 at 0x801C2998/0x801C299E) swept the spawn point diagonally right→left, start/end Y (200/−200 at 0x801C299A/0x801C29A0) swept low→high, start/end Z (−100/+100 at 0x801C299C/0x801C29A2) swept left→right — all smooth sweeps matching curve 0's ramp
  - R: `godot-learning/tools/parse_effect.py:504-505` (start x/y/z @ +0x14/+0x16/+0x18, end x/y/z @ +0x1A/+0x1C/+0x1E) + `:566` (position curve = byte 0x08 low nibble) + `godot-learning/src/effects/ActiveEmitter.gd:101,275` (spawn position = `interpolate_vec3(start, end, curve_t)`); validated by `godot-learning/tests/EffectSoundCaptureTest.gd` (drives E001 through the parsed pipeline)
  - src: `research/working_documents/CURVE_ANALYSIS.md`
  - src: `research/working_documents/VERIFIED_spawn_position_curve.md`
- **The spread curve (high nibble of emitter byte 0x08) is resolved by FUN_801a8c8c(emitter, factor, &out), whose XYZ output is used as ±rand() bounds that scatter each particle around the interpolated spawn center.** — `[S·D·R] 3/3`
  - S: FUN_801a8c8c call and rand() scatter in FUN_801a60ac, per `research/working_documents/CURVE_ANALYSIS.md`
  - D: E001 emitter 2 runtime write test (2025-11-20): `write 0x801C298C 0x02 0x01 0x00 0x00` → `0x12 0x01 0x00 0x00` switched the spread-curve nibble 0x0 (no curve, static — all particles spawned at the exact same point with zero spread) to 0x1 (curve 0), and the observed scatter progressively widened over the animation
  - R: `godot-learning/tools/parse_effect.py:567` (spread curve = high nibble of byte 0x08 via decode_curve_index) + `godot-learning/src/effects/ActiveEmitter.gd:102,228-241` (spread vector interpolated by the curve's t, then `_apply_spread` ±random scatter per particle); validated by `godot-learning/tests/EffectSoundCaptureTest.gd`
  - src: `research/working_documents/CURVE_ANALYSIS.md`
  - src: `research/working_documents/VERIFIED_spread_curve.md`
- **The emitter's spread parameters are int16 little-endian at offsets +0x20/+0x22/+0x24/+0x26 (diagonal spreads at +0x20/+0x24, untested; spread_vertical at +0x22, spread_horizontal at +0x26 — E001 emitter 2 at RAM 0x801C29A4–0x801C29AB), and the spread radius interpolates per-frame as spread_radius = start + ((end − start) × curve[anim_frame] / 255), with each particle placed at center ± random(0, spread_radius).** — `[S·D·R] 3/3`
  - S: E001.BIN emitter 2 spread fields at emitter +0x22/+0x26 (diagonal +0x20/+0x24) with base RAM 0x801C2984, and the lerp/scatter formula, per `research/working_documents/VERIFIED_spread_curve.md`
  - D: E001 emitter 2 runtime write test (2025-11-20): `write 0x801C29A4 0x00 0x00 0x32 0x00 0x00 0x00 0x64 0x00` (spread_vertical 50, spread_horizontal 100) + `write 0x801C298C 0x12 0x01 0x00 0x00` (spread curve 0) — original all-zero config spawned every particle at the exact same point; the modified config showed smooth progressive widening of the particle cloud to ≈50 vertical / ≈100 horizontal scatter
  - R: `godot-learning/tools/parse_effect.py:508-509` (spread start s16 at +0x20/+0x22/+0x24, end s16 at +0x26/+0x28/+0x2A) + `godot-learning/src/effects/ActiveEmitter.gd:102` (spread lerped by the curve's t before `_apply_spread` scatter); validated by `godot-learning/tests/EffectSoundCaptureTest.gd`
  - src: `research/working_documents/VERIFIED_spread_curve.md`
