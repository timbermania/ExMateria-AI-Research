...
- **A `{Delay}` immediately before the final glyph is NOT applied on the PSX: the Orbonne prayer's trailing `{Delay 3C}` (60u) sits before the last `.` yet the `.` blits only 10 VBlanks after the preceding `e` (predicted 70 if applied) — the final glyph flushes at base cadence (mechanism, end-of-message flush vs emit batching, unconfirmed).** — `[D] 1/3`
  ...
  - src: `research/working_documents/TYPEWRITER_TEXT_CADENCE.md`
- **The PSX does NOT gate event-script dispatch on the typewriter — the `Dialog=0x09` overlay is a fire-and-forget fiber: in the Orbonne prayer, Map Darkness (`0x1a`) fires 1.4 s after dispatch (~32 % typed) and Ovelia's next Unit Anim (`0x11`) 3.9 s after (~40 % typed), while the last glyph blits at +8.2 s.** — `[S·D·R] 3/3`
  - S: `wait_for_instruction_halt_decode.md` §3.5 (static proof, doc §4) — opcode `0x10` calls `FUN_8014c8a0(slot, FUN_801308c0)`, which spawns the typewriter as a cooperative fiber (entry stored at slot+0x44, active flag slot+0x48=1) and falls through to the next opcode with no main-thread yield; the `FUN_8014ca80` per-frame yields live inside the fiber, so a halt may live only on a Wait-family opcode
  ...
  - D: `display_message_overlay_decode.md` round-2 (2026-06-30): `event_dialogue_tick` fired exactly 86 times = the `Wait 0x56` (86 vsyncs) that follows the prayer — the script reached and ran the post-prayer Wait during the capture window rather than parking on the Display Message; the prayer's real wait is the later `Wait For Instruction Task=1` at chunk PC 50 (kind-1 predicate on the active typewriter)
