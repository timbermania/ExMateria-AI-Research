...
- **Four further effect-subsystem globals are mapped: 0x801BBF74 (g_sound_section_ptr, feds), 0x801BC0DC (g_sound_data_base), 0x801BAD0C (g_effect_context), and 0x801B48D0 (effect_base_lookup_table).** — `[S] 1/3`
  ...
  - src: `research/key_documents/LUA_DEBUGGING.md`
- **A ninth effect-subsystem global, script_bytecode_ptr at 0x801BC094, points into the loaded effect's script bytecode (opcode) section, reachable from callbacks via `lui 0x801C` + `lw -0x3F6C(v0)`.** — `[S·R] 2/3`
  - S: global 0x801BC094 = script_bytecode_ptr ("Script opcodes"), per `research/working_documents/WRITING_MIPS_CALLBACKS.md`
  - R: `effect-editor/mips/known_functions.lua:275` (symbol table entry 0x801bc094 = "script_bytecode_ptr"; no automated test)
  - src: `research/working_documents/WRITING_MIPS_CALLBACKS.md`
...
- **Timeline callback routing: dispatch_unit_triggers_and_register_callback (0x801A3D30) reads action_flags from the keyframe at offset 0x4C and, when bits 0–2 ≠ 0, sets effect_state->callback_state[slot] = 1 (offset 0x22) and copies callback_slots[slot] into effect_state->callback_ptrs[slot] (offset 0xD4); process_particle_channel_and_unit_triggers (0x801A4000) then reads the channel keyframe's action_flags at offset 0x4A and, when bits 0–2 ≠ 0, JALRs callback(effect_target, slot, emitter_index − 1, spawn_counter) at 0x801A4134 instead of the normal emitter spawn.** — `[S·R] 2/3`
  ...
  - S: callback argument ABI — a0 = effect_target (int16 effect instance ID, sign-extended to 32-bit), a1 = callback slot (0–3), a2 = triggering emitter_index − 1, a3 = spawn counter; return value in v0, usually ignored, per `research/working_documents/WRITING_MIPS_CALLBACKS.md`
...
- **Pattern 2 effects spawn particles at a single position (effect_target_index); the Pattern 2 corpus scripts are uniformly 36 bytes except E225 "Very Quick Bloody Strike" at 32 bytes.** — `[ ] 0/3`
  ...
  - src: `research/working_documents/PATTERN2_EFFECTS.md`
- **The effect runtime state structure is at least 0xD4 (212) bytes and may be larger; it is passed to update_all_particles and the particle handler functions.** — `[ ] 0/3`
  - R: none — no 0xD4 byte-offset runtime state struct in godot-learning (probed godot-learning/src/effects; EffectInstance/ParticleSubsystem model the effect as typed objects, not a byte-offset struct)
  - src: `research/working_documents/WIKI_EFFECT_STATE_TABLE.md`
- **The effect runtime state's effect_index field (0x02–0x03) maps to the E###.BIN file number (1 = E001.BIN Cure) and is used to look up effect-specific data tables.** — `[ ] 0/3`
  - R: none — no 0x02 effect_index field in godot-learning (probed godot-learning/src/effects; the reimplementation maps abilities to E###.BIN files via `godot-learning/src/data/AbilityDatabase.gd` `effect_id`/`effect_file`, not a runtime struct field)
  - src: `research/working_documents/WIKI_EFFECT_STATE_TABLE.md`
- **The effect runtime state's tick_counter field (0x06–0x07) is incremented by 2 (not 1) on each call to update_all_particles.** — `[ ] 0/3`
  - R: none — tick_counter not present in godot-learning (probed godot-learning/src, godot-learning/tests)
  - src: `research/working_documents/WIKI_EFFECT_STATE_TABLE.md`
- **The effect runtime state's frame_counter field (0x20–0x21) is the animation frame counter passed to emitter_control_routine for spawn timing.** — `[R] 1/3`
  - R: `godot-learning/src/effects/ParticleSubsystem.gd:324,367,386-399` (spawn-interval gating on effect_frame, comment: "PSX uses effect_state->frame_counter for spawn interval gating, not particle age") + `godot-learning/tests/EffectSoundCaptureTest.gd` (drives E001/E065 through the parsed pipeline)
  - src: `research/working_documents/WIKI_EFFECT_STATE_TABLE.md`
- **op_animate_tick (opcode 40) does NOT check max_keyframe for particle channels — keyframes advance based purely on time markers — while op_process_timeline_frame (opcode 41) DOES check it, via process_particle_channel_and_unit_triggers; the timeline header words (phase1_duration, spawn interval, phase2 start delay) are consumed only by opcode 41, not by opcode 40.** — `[S·R] 2/3`
  - S: max_keyframe check inside process_particle_channel_and_unit_triggers (doc cites 0x801A405C–0x801A406C in that function); op_animate_tick 0x801A3408 / op_process_timeline_frame 0x801A4838, per `research/working_documents/WIKI_TIMELINE_UPDATES.md`
  - R: `smd-player/addons/exmateria_sound/runtime/effect_sound_controller.gd` (parent phase1/phase2 channels built with `enforce_max=true`, for_each children with `enforce_max=false` — sound-channel mirror of the same asymmetry) + `godot-learning/tests/EffectSoundCaptureTest.gd` (for_each onset gated on phase1_duration)
  - src: `research/working_documents/WIKI_TIMELINE_UPDATES.md`
- **Both op_animate_tick (40) and op_process_timeline_frame (41) pre-increment the particle-channel keyframe index before reading emitter_id and action_flags, so emitter_ids[0] and action_flags[0] are NEVER READ by either opcode, time_markers[0] is only used to calculate the duration of keyframe 1, and index 1 is the first usable keyframe.** — `[S·R] 2/3`
  - S: keyframe_index pre-increment in process_particle_channel_and_unit_triggers 0x801A405C–0x801A406C, per `research/working_documents/WIKI_TIMELINE_UPDATES.md`
  - R: `godot-learning/src/effects/PhaseBlock.gd` (`current_keyframe` starts at 1 — "skip kf[0]"; no dedicated test, GPURiseTimingTest.gd monitors PhaseBlock playback only) + `effect-editor/ui/timeline_tab.lua` (renders index 0 as "(unused)" with no controls)
  - src: `research/working_documents/WIKI_TIMELINE_UPDATES.md`
- **op_animate_tick's particle channel 4 state overlaps color track 0's state in effect_state: when channel 4's time markers are all identical (duration 0), its keyframe counter runs away and corrupts the color track's keyframe, so color tracks never process and the effect plays nothing even though particles should spawn — a quirk of the ROM's memory layout, not a defect in the effect data.** — `[ ] 0/3`
...
- **`FUN_801a3c08` (battle.bin) is the timeline-frame walker: its inner loop holds the 5-channel animate_tick cursors at entity+0x2A+N*2 / entity+0x44+N*2 and a 4-entry per-track cursor block at entity+0x3A..0x40 / entity+0x54..0x5A — and that per-track block holds four track-type cursors (sound / palette / camera / screen), not four sound-channel cursors (all three haste entities carry the identical `[48, 4, 4, 0]` kf vector, out of range for a 4-keyframe track, and only one of the 12 cells across the entities is shaped like a live mid-keyframe cursor).** — `[S·D] 2/3`
  - S: `FUN_801a3c08` inner-loop cursor loads/stores — battle.bin disassembly (doc §"What the first run on haste taught us")
  - D: `diag_animation_vm_snapshot` capture + `diag_timeline_cursor_evolution` sweep (haste, 2026-05-23/24): raw hex byte-for-byte agreement with the disasm layout; the `[48,4,4,0]` vector static and identical across entities 0x80038B80 / 0x800387F0 / 0x800370E0 while only entity 0x800370E0's state evolves
  - R: none — per-track cursor block (+0x3A..0x40 / +0x54..0x5A) not present in godot-learning or smd-player (probed both; the landed snapshot consumer treats the audio entity's +0x3A family as residue camera/palette state and ignores it)
