...
- **`Block Start` (0x2A) allocates a free slot (1..15) via `FUN_80149bec(0x10)`, initializes that slot's coroutine with the generic block-body interpreter `FUN_8013e904` via `FUN_8014c8a0`, plants the byte right after the 0x2A as the slot's bytecode pointer, and then has the main thread fast-forward its own pointer past the matching 0x2B via `FUN_80149ebc`.** — `[S·D] 2/3`
  ...
  - ⚠ REFINED (2026-08-21): `FUN_80149BEC` **does not always allocate**. It is `allocate_task_slot(n)`: `slti v0,a0,0x10` then `beq v0,zero` — when `n < 16` it returns `n` **unchanged** (`j 0x80149C34` with `addu v0,a0,zero` at `ram:80149C00`), and it scans slots 1..15 for a free one only when `n >= 16`. Every call site documented in this vault passes the literal `0x10`, so every existing note is right about the case it documents and the *model* behind them is wrong — it is correct by accident at `0x10` and wrong on the first opcode whose hint is smaller. That matters because most opcode bodies hand it **one of the instruction's own parameter bytes**, so a script can name the slot its fiber goes in; the fetch at `ram:80143D0C` preloads the 4 parameter bytes into `s2`/`s5`/`s6`/`s7`. [[Display Message Opcode]] already records exactly this for `{10}` ("a DIRECT message-slot index 0..15 with sentinel `0x10` = auto-pick first free slot") — the rule was in the vault and had not been generalised. **A second allocator exists** that this vault has no point on: `0x80149C48` scans **upward from the running slot** rather than from 1, and its only callers are `{3B}` SpriteMove and `{6E}` SpriteMove beta — which is why SpriteMove reaches higher slots in a busy scene (web-psx measured 42 lives across slots 2..8, never 9+, against 2..5 for the scan-from-1 allocator's customers). Verified off `project-assets/fft-extract/BATTLE.BIN` (ExMateria-AI-Research#5)
