...
- **The `(row, col) → who` identity of an EVTFACE cell is recoverable from the game's own script: every scripted EVTFACE portrait is shown by a `{10}` message whose decoded text opens with the speaker's `{Color 08}` highlighted name header, so cell `(row, Portrait−1)` = that header's name; the mapping is one-to-one (43 of the 64 cells are ever shown — byte-for-byte the hand-authored per-event Row/Column table, the other 21 cells are filler/duplicates) while identity → cells is one-to-many (e.g. Dycedarg owns (0,3) and (5,1)); this supersedes the earlier ENTD-speaking-Unit axis, which is wrong because the Unit byte is scene-local and misses SPR-less identities (Balbanes, Draclau, Goltana, Elidibs).** — `[R] 1/3`
  ...
  - src: `research/working_documents/PORTRAIT_ROW_OPCODE_50_EVTFACE.md`
- **In `Portrait=0` boxed dialog the face is the SPEAKER UNIT's own `BATTLE/<sprite>.SPR` portrait region — file offset `0x8200` in the 256×32 portrait row, face at X=80..127 (48×32) stored ROTATED 90° → 32×48 on screen, CLUT row 8, indexed by `*(*(unit+0x134)+0x18a)` (the speaker's roster slot via `FUN_8008cdd0`) — and EVTFACE.BIN is ruled out as the source in scenario 1 (0 of 824 verbatim 16-byte needles resident in VRAM, against 480/525 for the FRAME.BIN positive control; EVTCHR's chapel load is a single body-pose block, see [[EVTCHR Frame Resolution]]).** — `[S·D·R] 3/3`
  - S: portrait index `FUN_8008cdd0 @ 0x8008cdd0` = `*(*(unit+0x134)+0x18a)` (`battle_decompilation.c`); static-agent render of `HIME.SPR` portrait across its 16 CLUTs → Ovelia's face at row 8 (2026-06-27)
  - D: full-VRAM dump needle-match `last_run/vram_dump.bin` off `orbonne_prayer_mid_dialog.sstate` (2026-06-27): EVTFACE.BIN 0/824, FRAME.BIN control 480/525, EVTCHR.BIN 4145/36196 (event field-sprite chr, partial)
  - R: `godot-learning/tools/extract_spr.py` (PORTRAIT_OFFSET=0x8200, PORTRAIT_REGION_X=80, width 48, selective portrait-row palette decode) + `src/ui3/elements/UIPortrait.gd` unit-SPR path (stored-rotated region → 32×48 display) — validated by `godot-learning/tests/DialogueBoxTest.gd` `_test_portrait_size_dock_facing`
  - src: `research/working_documents/scenario_1_captures/boxed_dialog_decode.md`
- **On a cold scenario load the chapel units' portraits are ASSEMBLED into a VRAM TPAGE column at X=832: eight 64×32 tiles stacked Y=256→480 (one per unit), all uploaded from the SAME RAM staging buffer `0x801e7200` that is refilled per portrait and re-uploaded to the next tile (GsLoadImage wrapper `ra=0x80022cd8`); the dialog's 31×48 face quad samples the middle of a tile, and the uploads re-fire only on a cold load — in-session savestate reloads skip already-resident blocks.** — `[D] 1/3`
