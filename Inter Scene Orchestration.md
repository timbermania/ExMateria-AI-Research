...
The layer above a single scenario's playback: who computes the next scenario_id, and which code tears down the current world and boots the next one. As of the 2026-07-10 scn6→7/8 capture the cinematic chain is resolved — the ATTACK.OUT overlay (not BATTLE.BIN) owns a selector that linear-scans the live scenario table and chains on the matching record's `post_scenario_step`/`next_scenario_id` (field theory confirmed live: scn6's `0x81`/`7` drives the chain to the root-7 group, settling on member 8). The battle path instead picks its successor from BattleConditionals bytecode (`{0x0019} Run Scenario N`), and the handoff is gated by a scene-end fiber-join barrier (`FUN_80145f78`), and the word at `0x8016A014` is the scenario-interpreter fiber's stack mirroring the current scenario id — whose home is event-script variable `0x27` at `0x800577B8`, written by `{0x0019} Run Scenario N` via `event_set_script_variable` (`0x8013B644`; 2026-08-22 correction). Still open: the `0x80` world-map fork, the selector's exact match predicate, and the single instruction that commits the id flip.
...
- **`0x8016A014` is the event-VM execution-context base, NOT a clean story scenario counter: its low byte equals the current scenario id at rest, but during a transition the fiber scheduler churns the whole dword (observed sweeping 4..15), so a Write-BP on it cannot isolate "the write that selects scenario 7"; the context struct is owned by the scene-end join fiber — its return sites `0x80145f9c`/`0x80145fc4`/`0x80145fe0` are exactly the pointers stashed in the struct at +0x1C/+0x20.** — `[S·D] 2/3`
  ...
  - ⚠ SUPERSEDED (2026-08-22) by: the current scenario id's home is event-script variable `0x27` at `0x800577B8` (`*(0x80165F9C) + 0x27*4`), written by `{0x0019} Run Scenario N` via `event_set_script_variable` (`0x8013B644`); `0x8016A014` is a word inside cooperative-task slot 1's fiber stack (the scenario interpreter's) that mirrors the id and can be stale — it disagrees with var `0x27` on 4 of the 31 battle-side captures, three of them with the interpreter fiber idle (per `SCENARIO_LOADING.md` §3.2.6)
  - src: `research/working_documents/INTER_SCENE_ORCHESTRATION.md`
...
- **Battle-outcome chaining is the BattleConditionals bytecode instruction `{0x0019}` "Run Scenario N": BC[1] is resident at RAM `0x80049A18` (= `BTLEVT.BIN` +0x542, byte-exact), with `Run Scenario 5` @ BC[1]+0x2E on battle-start and `Run Scenario 6` @ +0x3A on victory — matched the observed `0x8016A014` counter ticks 4→5→5→6, so a battle scenario picks its successor from its BC set.** — `[D·R] 2/3`
  ...
  - ⚠ SUPERSEDED (2026-08-22) by: the `BTLEVT.BIN` +0x542 match at `0x80049A18` is 512 bytes, not the 1537 of the dump — the installer (`ATTACK.OUT` file+`0x3ECC`) copies 256 halfwords, and bytes past that are unrelated RAM; the largest in-game BC set is 380 bytes, so the 512-byte bound is fine (per `SCENARIO_LOADING.md` §3.2.5)
  - src: `research/working_documents/INTER_SCENE_ORCHESTRATION.md`
...
- **Scenario 1's dialogue advance triggers NO new disk loads: 5 CIRCLE (D) presses on a paused dialogue page leave the `caseD_5` exec-BP hit count at 36 → 36, so the dialogue text + portraits + speaker names are already in RAM before the first page is shown — either the scenario-1 event chunk lives inside BATTLE.BIN (the 1.5 MB region at `0x80067000`–`0x801BF000`) or inside ATTACK.OUT (128 KB at `0x801BF000`), or `EVENT/TEST.EVT` was loaded via a path the BP doesn't cover.** — `[D] 1/3`
  - D: 2026-06-20 capture (sstate2 + START + 48 s): 36 LBA-loads via `FUN_80011E38` caseD_5; hit count unchanged across 5 D-presses on the paused Agrias page ("The guards have already arrived.") (per `SCENARIO_LOADING.md` §3)
  - R: none — no CD-load-on-advance in godot-learning (chunk refresh is JSON-file based from `assets/scenarios/chunks/`; probed `src/scenarios/`)
  - src: `research/working_documents/SCENARIO_LOADING.md`
