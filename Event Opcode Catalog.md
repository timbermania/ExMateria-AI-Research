...
- **Per the wiki's "Unused/Unknown" table these catalog IDs have no confirmed behavior — `{12}`, `{23}`, `{24}`, `{25}`, `{26}`, `{37}`, `{39}`, `{3A}`, `{3F}`, `{40}`, `{5C}`, `{5D}`, `{61}`, `{62}`, `{66}`, `{6C}`, `{6D}`, `{71}`, `{72}`, `{73}`, `{74}`, `{75}`, `{81}`, `{82}`, `{8F}`, `{93}`, `{95}`, `{C0}`, `{D8}`, `{D9}`, `{DA}`, `{DC}`, `{F0}` — of which `{23}`, `{24}`, `{25}`, `{26}`, `{37}`, `{3F}`, `{5C}`, `{5D}`, `{61}`, `{62}`, `{6C}`, `{72}`, `{74}`, `{81}`, `{95}`, `{C0}`, `{D8}`, `{D9}`, `{DA}`, `{DC}` are "Never used" in any retail event.** — `[S] 1/3`
  - ⚠ SUPERSEDED (2026-08-21) in part by: **`{12}` is not unknown** — it is the per-unit graphic/palette **restore pass**. Its handler is the fall-through at `ram:8014429C`, reached when the dispatcher's compare chain tests the opcode against the `ori v0,zero,0x12` at `ram:80144254` (`ram:80144290 bne s4,v0 -> 801442AC`). The body sign-extends the low 16 bits of `s3` into `a0` (`sll`/`sra` at `ram:80144298`/`ram:801442A0`), calls `FUN_80147D98` — which walks the 21 roster slots, matching each unit's event-id byte `+0x161` and flag `+0x5 & 0x30`, and calls `FUN_8008D26C(slot, 2, -0x1F, -0x1F)` — then `j 0x80145F24` → `j 0x80143D0C`, back to the fetch loop. This body was previously attributed to `{DB}` in [[Event End Opcode]]; that attribution is superseded and the description moves here. Re-derived off `project-assets/fft-extract/BATTLE.BIN` (ExMateria-AI-Research#5)
