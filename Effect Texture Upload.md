...
- **CLUT field encoding (16-bit, at primitive offset 0x0E): bits 0–5 = VRAM X ÷ 16, bits 6–14 = VRAM Y (e.g. VRAM (0, 492) = (492 << 6) | 0 = 0x7B00); TPAGE field encoding (16-bit, at offset 0x1A): bits 0–3 = TX (page X = TX×64), bit 4 = TY (page Y = TY×256), bits 5–6 = ABR (semi-transparency mode 0–3), bits 7–8 = TP (color mode: 0=4-bit, 1=8-bit, 2=15-bit) — e.g. an 8-bit texture at VRAM (384, 0) with ABR=1 gives TX=6, TPAGE = 6 + 0 + 32 + 128 = 0x00A6.** — `[S·R] 2/3`
  - S: BATTLE.BIN disassembly 0x801A5650–0x801A5658 (multi-sprite loop copies the frame's `texture_page` field verbatim into the primitive's TPage field) + E019.BIN texture_page 0x00A6, per `research/working_documents/PSX_TPAGE_RESEARCH.md`
  - R: godot-learning/tools/parse_effect.py:814–817 (x_base = tp & 0x0F, y_base = tp >> 4 & 0x01, blend = tp >> 5 & 0x03, color_depth = tp >> 7 & 0x03) + effect-editor/core/parser.lua `decode_texture_page` (same layout) — no named validating test
  - src: `research/key_documents/CUSTOM_EFFECT_HOOKS.md`
