...
- **The D9 pitch-LFO arm (`smd_op_d9_lfo` @ `0x800164D4`) encodes byte1 as the signed rate, byte0 as the step, and byte2 as mode bit 0x10 + wf_idx low nibble — when rate != 0 it writes the sign-preserved `rate² << 14` step source (from `pitch_lfo_step_calc` on `rate << 14`) to `chan+0xEC`, the step to `chan+0xF2` (the countdown's `inner_reload`), `0x100` to `chan+0xFA` (depth_reload), byte2 to `chan+0xFD`, `0` to `chan+0xFC` (the mode byte the handler's mode dispatch reads), the active flag `!mode_bit<<1 | 1` to `chan+0xFE`, and `PTR_LAB_80028F54[wf_idx]` to `chan+0xE0` (the callback ptr) before resetting the countdown; when rate == 0 the whole arm is skipped (`beq a0, zero` @ `0x80016500`) and no fields are written, so no LFO callback is armed.** — `[S·D·R] 3/3`
  - S: `FUN_800164D4` D9 listing — `lb a0, 0x1(s4)` / `lbu s2, 0x2(s4)` / `lbu s3, 0x0(s4)`, rate-zero skip @ `0x80016500`, `sll a0, a0, 0xe` + `jal pitch_lfo_step_calc`, `sw v0, 0xEC(s1)` @ `0x80016544`, `sh v0, 0xFA(s1)` @ `0x80016550`, `sh s3, 0xF2(s1)` @ `0x80016558`, `sb s2, 0xFD(s1)` @ `0x80016570`, `sb zero, 0xFC(s1)` @ `0x80016574`, `sh s0, 0xFE(s1)` @ `0x80016578`, `sw v0, 0xE0(s1)` @ `0x80016580` — doc §5.1 MIPS listing
  - D: `probe_opcode_d9_lfo` (BP `0x800164D4`) PAIR on `cure_4_no_music` — D9 params dispatch identically on both engines, proving the wf_idx reaches the dispatcher (2026-05-14)
  - R: `smd-player/addons/exmateria_sound/runtime/shared/opcodes/lfo.gd` (0xD9 handler: `lfo_sub_step_source[0]` = sign-preserved rate²<<14, `lfo_sub_inner_reload[0]` = step (clamped ≥ 1), `lfo_sub_depth[0]`/`lfo_sub_depth_delta[0]` = 0x100, `lfo_sub_callback_idx[0]` = byte2 & 0x0F, `lfo_sub_active[0]` = 1, `lfo_sub_countdown[0]` = 0) — validated by the `probe_opcode_d9_lfo` pair entry in `smd-player/workspace/orchestrator/probe_validation_manifest.py`
