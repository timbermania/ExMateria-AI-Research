...
- **`{7F}` EVTCHR Palette is a timing gate, not a palette setter: the handler `FUN_8014a3f8` partial-decodes to an `slt v0, v0, s0` gate plus a yield/wait (`FUN_8014ca80`), the chapel cinematic chunk contains ZERO 0x7F opcodes (only Load EVTCHR @ PC 5 and Save EVTCHR @ PC 7), and live `unit[+0x13F]=0` (no XOR override) for all units.** — `[S·D] 2/3`
  ...
  - S: spin body `while (FUN_8013b590(var_id) < threshold) Thread_doNext()` per tick; `FUN_8013b590` is the generic event-VM variable getter (built from `FUN_8014a018(0xBE)` + `(0xB1,id)`), mirror setter `FUN_8013b644`; the counter is bumped by the palette-upload pass (writer not fully traced — static only) (per `research/working_documents/scenario_1_captures/evtchr_load_save_decode.md`, 2026-06-27)
...
- **In Godot the cinematic palette row is set at spawn from the ENTD palette byte — `unit.body_palette_row = int(slot.get("palette", 0))` in `ScenarioPlayerScene._spawn_units()` (`ScenarioPlayerScene.gd:264`) — and the cinematic path (`SpriteLayerManager.enter_cinematic_mode` / `load_cinematic_frame`, `SpriteLayerManager.gd:539`) swaps ONLY the pixel atlas, deliberately keeping `type1_palette` + `body_palette_row` bound to the unit's own SPR.** — `[R] 1/3`
  ...
  - R: re-confirmed by `research/working_documents/scenario_1_captures/evtchr_palette_pipeline.md` (V14 re-landed 2026-06-27): the cinematic walker never picks a palette; `enter_cinematic_mode(seg_id)` swaps only `type1_tex` + `type1_tex_size`; the `{7F} EVTCHRPalette` override is the only path that would swap in an EVTCHR-embedded palette, not exercised in chapel
  - src: `research/working_documents/chapel_opcode_trace/HANDOFF_sprite_palette_resolution.md`
...
- **Godot's baked cinematic palette atlas `segment_000.palette.tga` packs the chapel mains' palettes in its low rows — row 0 = Agrias (AGURI.SPR), row 1 = the priest (SIMON.SPR), row 2 = Ovelia (HIME.SPR), rows 3–6 the other chapel cast, rows 7–15 all zero — with the trio's row order the REVERSE of their ENTD always_present slot order (HIME slot 0 → row 2, SIMON slot 1 → row 1, AGURI slot 2 → row 0); each trio row bit-matches its SPR's `palette[0]` at distance 0.** — `[D] 1/3`
  ...
  - Refinement (2026-06-27, `research/working_documents/scenario_1_captures/evtchr_palette_pipeline.md` decode): rows 7..15 are not byte-zero — each holds exactly one non-zero entry, index 0 = green sentinel `(0,104,0)`, indices 1..15 black; "all zero" holds for the 15 color slots
...
- **In the 60-s chapel capture window every LoadImage VRAM write (341 calls, 17 caller PCs) lands in fixed regions: unit SHP sprite frames via `0x80067820` at (X, 8|248, 24×224) — 120 hits —, dialog font glyphs via `0x8012fb40`, the HUD strip via `0x80093024` at (0, 494, 256×14), effect-palette init via `0x801c8b9c` (one 16-color palette repeated across 16 CLUT slots at y≈224..227), and the ONLY EVTCHR-segment-sized uploads — four 64×256 blocks via the `0x800f2` cluster (case dispatch `0x800f2d70`/`dc0`/`e10`/`e60`) to (768..1023, 0..255), sourced from 32 KB segments at `0x801df000`/`0x801e7000`/`0x801ef000`/`0x801f7000`.** — `[S·D] 2/3`
  ...
  - src: `research/working_documents/scenario_1_captures/clut_upload_decode.md`
- **EVTCHR segment 0's embedded palettes (the chapel sheet) carry the cream-white robe color in index 2 of ALL painter rows 0..6 (≈ (216–224, 216–224, 192–216) 8-bit after PSX-TIM 5-bit unpack, e.g. (224,216,208) row 0) — the single slot reused for every robe / veil / candle-light plane of the chapel atlas; per-character variation rides on indices 3..14 (trim / hair / skin), so even an off-by-one palette-row pick still renders white robes and the bug surfaces in skin and trim, not the dominant fill.** — `[S·R] 2/3`
  - S: EVTCHR.BIN segment 0 embedded palettes @ +0x0780, rows 0..6 decode samples (source doc static decode; `godot-learning/tools/parse_evtchr.py` `decode_palette`)
  - R: `godot-learning/tools/parse_evtchr.py` (`decode_palette` / `decode_pixels`, the "fully parsed" pipeline) + `godot-learning/tools/test_parse_evtchr.py` (segment-0 palette 16×32 B + 4bpp pixel assertions)
  - src: `research/working_documents/scenario_1_captures/evtchr_palette_pipeline.md`
- **EVTCHR segment 0's embedded-palette rows 7..15 are placeholders, not painters: each holds exactly one non-zero entry — index 0, a green sentinel `(0,104,0)` — with indices 1..15 black, so any unit whose body palette row lands there renders as a flat silhouette/blob.** — `[S] 1/3`
  - S: EVTCHR.BIN segment 0 embedded palettes @ +0x0780, rows 7..15 decode samples (row 7 = `[(0,104,0), (0,0,0), ...]`, nonzero=1; source doc; `godot-learning/tools/parse_evtchr.py` `decode_palette`)
  - R: none — the doc's Godot-side demos (F3 `cinematic_palette_row` row pin at `ScenarioVMDebugPanel.gd`, the `SpriteLayerManager.gd` row ≥7 silhouette warning) are gone after the 2026-06-27 V14 re-land; probed `godot-learning/src/`, `godot-learning/tests/` for `cinematic_palette_row` / `resolve_unit_palette` — absent
  - src: `research/working_documents/scenario_1_captures/evtchr_palette_pipeline.md`
- **Godot bakes each EVTCHR segment as a TGA pair: `segment_NNN.tga` (256×200 RGBA, every texel `(idx*17, idx*17, idx*17, 255)` so the unit shader recovers the 0..15 index as `int(pixel.r * 15.0)`) + `segment_NNN.palette.tga` (16×16 RGBA, row Y = palette index, column X = color index, sampled at `vec2(idx/15.0, body_palette_row/15.0)` under `filter_nearest`) — the same indexed-grayscale + 16×16 palette convention as the BODY-sprite path (`extract_spr.py`), so cinematics needed no new shader; the bake is idempotent and must be re-run after a fresh EVTCHR.BIN extract or the palette TGAs go stale.** — `[R] 1/3`
  - R: `godot-learning/tools/bake_evtchr_textures.py` (`bake_segment`, index×17 encode, 16×16 palette TGA layout) + `godot-learning/assets/shaders/unit.gdshader` (`texture(tex, uv).r * 15.0` index recovery, `type1_palette` `filter_nearest`); no bake-specific test named
  - src: `research/working_documents/scenario_1_captures/evtchr_palette_pipeline.md`
