...
- **The Orbonne `{33}` view-engine darkening is a uniform per-channel ADDITIVE offset with 5-bit clamp — `dark = clamp(source + offset, 0, 31)`, one offset triple per palette view (map band −4, sprite −8, unit +8; R/G byte-exact, B +1 floor residue) — armed by `{33}`/`{32}` via `FUN_8008f710` and walked by the view ticker `0x800917b0` under each view's `thr` enable byte (NOT via `{1A}` `FUN_80090840`, inert 01↔02, nor `{2E}`); this is exactly Godot's mode-4 `ScenarioColorTint` affine (`scale=1`, `bias=signed-delta`) + shader clamp, so only the ramp trajectory (Godot lerp vs PSX 32-step DDA) is sub-faithful (cosmetic; endpoints + duration already match).** — `[S·D·R] 3/3`
  - S: view ticker `0x800917b0`, color-command path `FUN_8008f710`, per-view `thr` gate (our disassembly, doc §8 Q3; full write-up `prayer_screen_tint_quad_decode.md` §11)
  - D: live CLUT diff across the three `orbonne_prayer_tint_*` savestates (2026-07-05): uniform per-channel offset per view, R/G byte-exact, B +1 floor
  - R: `godot-learning/src/scenarios/ScenarioColorTint.gd` mode-4 affine + shader clamp, validated by `godot-learning/tests/ScenarioColorFieldTest.gd` `_test_psx_parity_uniform_additive_clamp` (48/0)
