...
- **FFT's `lfo_handler_tick` (0x800174C8–0x800175E0) walks every chan-pool slot on every cadence but fires the sub-slot-0 LFO callback only when `chan_word_0 != 0` (PC 0x800174D0: `beq v0, zero, LAB_800175f4`) and the sub-slot-0 active bit `chan+0xFE & 0x1` is set (PC 0x800174EC) — EndBar clears `chan_word_0`, so once a channel's bytecode terminates the LFO is disarmed, PRNG advance included.** — `[S·D·R] 3/3`
  ...
  - D (additive): ice_no_music `probe_lfo_subslot0_state` V21 — PCSX pre-gate rows (BP @ `0x800174C8`, inner-loop entry before the active-gate check at PC `0x800174F0`) show `active_dir=0` on all 231 captures with static `accumulator=−16777216` across cad 0–519 (savestate had `active_dir=3`, cleared by `play_sound` init), while pre-fix Godot ran the mode-0 body every cadence with `active_dir=3` — gate-skip behaviour confirmed on a channel whose savestate carried a live sub_0 arm (2026-05-18)
...
- **FFT's effect-engine PRNG is a 32-bit xorshift at `FUN_800178F4` (0x800178F4–0x8001791C) on state `DAT_80032a18`; the noise-waveform pitch-LFO callbacks (wf_idx 6 = `FUN_8001780C`, wf_idx 7 = `FUN_80017878`, selected by D9 param[2]'s low nibble through jumptable `PTR_LAB_80028F54` — 9 entries, wf_idx 0–7 + default) each consume one PRNG value per firing, and wf_idx 6 computes `acc = (step_src >> 15) * prng` with `>> 15` a signed arithmetic shift (PC 0x80017850: `sra v1, v1, 0xf`) — e.g. `D9 04 D7 06`: byte1 0xD7 = −41 signed, rate² = 1681, sign-preserved lo = −1681, step_src = −1681 << 14 = −27,541,504.** — `[S·D·R] 3/3`
  ...
  - src: `research/effect_sound/working_documents/CAD_497_WF_IDX_6_PRNG_DESYNC.md`
- **The wf_idx=7 noise LFO callback (`FUN_80017878`) re-samples the engine PRNG only when the sub-slot countdown (sub+0x10) wraps to 0 — every tick it decrements the countdown and otherwise holds the accumulator — and on wrap it reloads the countdown from `inner_reload` (sub+0x12), pulls one PRNG value, and commits `acc = (step_source >> 14) * prng − step_source` into the accumulator (sub+0x04), with `>> 14` a signed arithmetic shift (PC `0x800178B8`: `sra v1, a0, 0xe`); for `D9 01 16 07` (step_p=1 → inner_reload=1, step_source 7929856) the PRNG therefore re-arms every tick and the high-16 pitch-bend contribution scatters ≈ ±121 randomly rather than oscillating.** — `[S·D·R] 3/3`
  - S: `FUN_80017878` @ `0x80017878`–`0x800178DC` — countdown `lhu/sh 0x10(s0)` + wrap-skip `bne v0, zero, LAB_800178CC`, `inner_reload` `lhu 0x12(s0)`, `jal FUN_800178F4` @ `0x800178A8`, `sra v1, a0, 0xe` @ `0x800178B8`, `mult`/`subu`/`sw 0x4(s0)` @ `0x800178B8`–`0x800178C8` — doc §5.2 MIPS listing
  - D: `probe_pitch_inputs` voice 19 (armed by the cad-241 `D9 01 16 07` wf_idx=7 D9) bit-exact vs PCSX for call_index 1..501 (cadence 0..419) with the unique pitch_bend value set identical to PCSX's, post-fix (2026-05-15)
  - R: `smd-player/addons/exmateria_sound/runtime/shared/per_tick/advance_lfo.gd` (wf_idx==7 branch: `sra_s32(step_src, 14) * prng_val − step_src` on the countdown-wrap tick, accumulator held otherwise) — validated by the `probe_pitch_inputs` pair (BP `0x80017368`)
  - src: `research/effect_sound/working_documents/VOICE_18_LFO_NOISE_WAVEFORM_MISSING.md`
...
- **Both engines seed the noise-LFO PRNG from the PCSX savestate's `DAT_80032a18` (0x71A4E3C8 on cure_4), so PCSX and Godot draw the same xorshift sequence — the seed is loaded on both sides from the `engine_prng_state` u32 emitted by the savestate extractor, and the shared-sequence claim is conclusive via voice-19 pitch_bend bit-exact over 501 consecutive rows.** — `[D·R] 2/3`
  ...
  - D (additive): the seed is the 4 bytes at PSX address `0x80032a18` in the savestate RAM dump; `extract_noise_seed.py` exposes it as the `engine_prng_state` JSON field and `render_effect_sound.gd` consumes it via `--noise-lfsr-seed=<json>` (2026-05-15, VOICE_18 doc)
