...
- **The disarm is bit-only: 0xE7 touches only bit 0 of `chan+0x11e` and never resets the LFO accumulator/step, so a later 0xE6 re-arm resumes from the pre-disarm accumulator value — consistent with the 0xE5 re-init path, where FFT's `pitch_lfo_period_reset` (PC `0x80016DC0`) resets the accumulator and reloads the countdown but does not zero `step_current`.** — `[S·D·R] 3/3`
  ...
  - src: `research/effect_sound/working_documents/RERAISE_VOICE_21_LFO_E7_DISARM_MISSING.md`
- **On `cure_4_no_music`, voice 19's vol-LFO (sub-slot 1, chan+0x88) runs a descending ramp with constant stride −2688 (`60160 → 57472 → 54784 → …`) after its initial ramp-up — the sub-slot callback flips the sign of its step accumulator; voices 18/20 carry no active vol-LFO on this effect.** — `[D·R] 2/3`
  - D: `probe_vol_inputs.jsonl` (PCSX, 723 rows), `cure_4_no_music` (2026-05-14) — voice 19 first 10 non-zero chan_88 rows show the −2688 descending stride (peak 62848 one stride above 60160)
  - R: `smd-player/addons/exmateria_sound/runtime/shared/per_tick/advance_lfo.gd` (sub-slot 1 per-tick ramp/swap advancing `chan_88_value`) + `runtime/shared/opcodes/e5_3param.gd` (0xE5 arm) — validated by the `probe_vol_inputs` `chan_88` pair on `cure_4_no_music` (0/264 mismatches post-fix)
  - src: `research/effect_sound/working_documents/VOL_FORMULA_INPUTS_PARITY_BUG_B_PLAN.md`
- **The vol formula's env term consumes the vol-LFO accumulator directly: FUN_80017118 reads it at PC `0x800171C8` (`lh v0, 0x88(s0)`) and adds it to the chan+0x98 dynamics accumulator inside the 0..0x7FFF clamp, while the pan-LFO accumulator feeds the pan arg at PC `0x80017214` (added to chan+0x90 + pool+0xAE) — the pre-fix Godot drainer read both LFO terms as hardcoded zero (`0x4000 + 0 + pan`; env sample without the chan_88 add).** — `[S·R] 2/3`
  - S: `0x800171C8` (`lh v0, 0x88(s0)`, env-term read) + `0x80017214` (`lh v1, 0x8a(s0)`, pan-arg read) in FUN_80017118 — `project-assets/fft-rom/scus_disassembly.txt` (doc §B1)
  - R: `smd-player/addons/exmateria_sound/runtime/shared/note_handler/compute_vol_lr.gd` (:38-40 `env_sample = clampi(chan_98_signed + chan_88_signed, 0, 0x7FFF)`; :54-58 `pan_base = clampi(0x4000 + chan_8a + pan, 0, 0x7F00)` — the doc's literal-0 patch sites now wired to `chan_88_value`/`chan_8a_value`) + `runtime/effect_sound/play_sound.gd` (`probe_vol_inputs` emit) — validated by the `probe_vol_inputs` / `probe_vol_lr_staging` pairs in `smd-player/workspace/orchestrator/probe_validation_manifest.py`
  - src: `research/effect_sound/working_documents/VOL_FORMULA_INPUTS_PARITY_BUG_B_PLAN.md`
