...
- **At scenario boot, `entd_to_roster_loader_16` (BATTLE.BIN 0x8017f8a0) walks the ENTD record selected by current_scenario_id and, for each non-empty slot, writes slot[+0x1] = roster_id and slot[+0x161] = unit_id, then calls `evtchr_queue_enqueue` (0x8008e540) at 0x8017fb88 with a0 = roster_id and Stack[+0x18] = unit_id; the queue is drained by `evtchr_unit_queue_drain` (0x80088e04) into `evtchr_slot_allocator` (0x80083cd4), whose choice — slot 12 for unit_id 0x0C, slot 0 for 0x13, slot 1 for 0x34 — is written to slot[+0x4] as roster_slot_idx; all five functions are labeled with [VERIFIED live] anchors in fft-ghidra/content/renames_high.tsv.** — `[S·D] 2/3`
  ...
  - D: chapel-boot producer BP capture (`orbonne_prayer_pre_scenario_load.sstate` + Enter, 2026-06-26, `clut_upload_decode.md` §V17.D.iii): `0x8017fb88` fires 3× (a0=0x08/0x05/0x05, s3=unit_id 0x0c/0x13/0x34), CLUT write `0x80087ba4` fires for slots 12/0/1 (0x78cc/0x78c0/0x78c1), sister `0x8008d05c` fires 5× for the chunk Add Units
  - src: `research/key_documents/ENTD_FORMAT.md`
...
- **The unit-add queue (`DAT_80049c1c`, 16 entries, stride 0x14, counter `DAT_80049c18`) has exactly four producers — `entd_to_roster_loader_16` (`0x8017fb88`), the 5-slot boot loader `FUN_8017f388` (`0x8017f550`), the `{45}` Add Unit handler (`0x8008d024`), and the sister Add handler `FUN_8008d05c` (`0x8008d0e0`) — and each entry's halfword[+0x0A] carries the producer's ENTD `unit_id` (the producer's Stack[+0x18]), which the drain `0x80088e04` reloads as the allocator's a0 (not the queue's byte[0], which an earlier handoff mis-traced).** — `[S·D] 2/3`
  - S: producer XREFs `0x8017fb88`/`0x8017f550`/`0x8008d024`/`0x8008d0e0`; dequeue `lh v0, -0x63da(at)` @ `0x80088da8` feeding `FUN_80087a28` Stack[+0x18], queue store `sh t3` @ `0x8008e66c` (`clut_upload_decode.md` §V17.B/D.iv, BATTLE.BIN disassembly)
  - D: chapel-boot BP capture on all four producer sites (`orbonne_prayer_pre_scenario_load.sstate` + Enter, 2026-06-26): `0x8017fb88` fires 3× (a0=0x08/0x05/0x05, s3=unit_id 0x0C/0x13/0x34), CLUT write `0x80087ba4` fires for slots 12/0/1 (0x78cc/0x78c0/0x78c1), `0x8008d05c` fires 5× for the chapel-chunk Add Units; heartbeat sanity 15054 hits
  - R: none — the four-producer queue / `DAT_80049c18` counter not present in godot-learning (probed `godot-learning/src/`, `godot-learning/tests/` for `0x8017f388`/`49c1c`; only a `0x8008d05c` doc-comment reveal mirror in `ScenarioPlayerScene.gd`)
