# Formation Screen Index

Domain index for notes on the FFT formation screen (the menu-screen roster/ability/equipment UI): its compositing/rendering model, per-cell element placement, and its panels and pickers.

## Notes

- [[Formation Screen Compositing]]
- [[Formation Element Placement]]
- [[Formation Ability Picker]]
- [[Equip Stat Delta Preview]]
- [[Formation Sort Tab Header]]
- [[Unit Pager Buttons]]
- [[Formation Vitals And Nameplate]]
- [[Roster Display Struct]]
- [[Selection Orb And Floor Spotlight]]
- [[Gold Selection Box]]
- [[Menu Window Box Open]]
- [[Start Action Menu]]
- [[Equip And Ability Panel]]
- [[Equip Sub Screen]]
- [[Change Job Screen]]
